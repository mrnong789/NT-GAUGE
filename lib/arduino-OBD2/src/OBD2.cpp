...
bool OBD2Class::pidSupportedExtra(uint8_t pid1,uint8_t pid2)
{
  if (pid1 == 0 || pid2 == 0) {
    return true;
  }

  return (_supportedPids[(pid1 - 1) / 0x20] & ((1UL << (31 - ((pid1 - 1) % 32))))) ? true : false;
}
...
float OBD2Class::pidReadExtra(uint8_t pid1,uint8_t pid2)
{
  ...
  if (!pidReadExtra(0x22, pid1, pid2, &value, sizeof(value))) {
    return NAN;
  }
...
int OBD2Class::pidReadExtra(uint8_t mode, uint8_t pid1,uint8_t pid2, void* data, int length)
{
  ...
  for (unsigned long start = millis(); (millis() - start) < _responseTimeout;) {
    if (CAN.parsePacket() != 0 &&
          ...
          (CAN.read() == (mode | 0x40) && CAN.read() == pid1 && CAN.read() == pid2)) {
